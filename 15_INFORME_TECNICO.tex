...
El protocolo SISER fue descubierto mediante decompilaci\'on de SunVision y permite leer datos de los 3 MPPTs del inversor trif\'asico (voltaje, corriente y potencia por cada MPPT), temperatura, voltaje y corriente de red, frecuencia, estado, y energ\'ia acumulada. El sistema est\'a \textbf{leyendo datos reales del inversor} desde junio 2026, con 22.000+ registros en TimescaleDB.
...
Este proyecto cubre:
\begin{itemize}
    \item Comunicaci\'on Modbus RTU con el inversor v\'ia RS232 (adaptador PL2303 USB-RS232)
    \item Protocolo de unlock del inversor Riello (secuencia de autenticaci\'on Modbus)
...
\begin{tikzpicture}[
    ...
    \node[box] (serial) at (0,-1.3) {RS232/USB\\PL2303 $\mid$ CH340 $\mid$ FT232};
    \node[box=green!10] (modbus) at (0,-2.6) {Modbus Reader\\(C + libmodbus)};
    \node[box=yellow!15] (buf) at (5,-2.6) {Buffer circular\\1000 muestras};
    ...
    \node[smallbox] (sunv) at (1.1,-8.7) {Eliminado\\(sunvision)};
    ...
    \draw[arr] (inv) -- (serial);
    \draw[arr] (serial) -- (modbus);
    \draw[arr] (modbus) -- (tsdb) node[midway, left, font=\scriptsize] {libpq};
    \draw[darr] (modbus) -- (buf);
    ...
    \node[left=0.2cm of modbus, font=\scriptsize, text=orange!70!black, align=right] {Unlock\\cada 5s};
\end{tikzpicture}
...
\midrule
Modbus Reader & C + libmodbus & Lectura del inversor & --- \\
...
Inverter Simulator & Python 3 & Simulador Modbus & 5502--5503 \\
sunvision-wine & Docker + Wine & SunVision via Wine & 8007 \\
...
\midrule
modbus-reader & gcc:12 (multi-stage) & Daemon C de lectura (legacy, reemplazado por siser-reader) \\
...
inverter-simulator & python:3.14 & Simulador Modbus RTU + SISER \\
sunvision-wine & --- & Eliminado del stack & --- \\
\bottomrule
\end{tabular}
...
\texttt{/sunvision/} & Eliminado & --- & Eliminada \\
\texttt{/v1sunvision/} & Eliminado & --- & Eliminada \\
...
\begin{enumerate}
    \item \textbf{Tiempo Real}: Potencia AC, temperatura, energ\'ia, voltajes y corrientes, actualizado cada 5 segundos. Muestra la \'ultima lectura y un gr\'afico de los \'ultimos 30 minutos.
    \item \textbf{Hist\'orico}: Energ\'ia diaria/mensual con barras apiladas, temperatura vs.\ potencia con ejes duales, eficiencia energ\'ica. Permite seleccionar rangos de fechas arbitrarios.
    \item \textbf{Diagn\'ostico}: Salud del sistema (\'ultima lectura exitosa, latencia del daemon, errores Modbus), voltaje y frecuencia de red, tiempo desde la \'ultima conexi\'on.
...
\subsection{Simulador del Inversor (Modbus RTU)}

Se implement\'o un simulador del protocolo SISER en Python, que permite ejecutar el sistema completo sin el inversor f\'isico. El simulador corre como contenedor Docker \texttt{inverter-simulator} y soporta los tres protocolos (Modbus RTU, NetMan UDP y SISER), siendo esencial para el desarrollo y testing: permite verificar que siser-reader, TimescaleDB y Grafana funcionan correctamente antes de conectar el inversor real.
...
\begin{itemize}
    \item \textbf{Xorg} + \textbf{Openbox}: Xorg es el servidor gr\'afico que permite mostrar ventanas en la pantalla, y Openbox es un gestor de ventanas extremadamente ligero. Juntos consumen menos de 50 MB de RAM, comparado con los 500+ MB que requerir\'ia un entorno de escritorio completo como GNOME o KDE. Esto es crucial porque el servidor lautaro tiene recursos limitados y ya ejecuta m\'ultiples contenedores Docker.
    \item \textbf{Chromium Browser en modo kiosco}: Se ejecuta con flags especiales (\texttt{--kiosk --noerrdialogs --disable-infobars --incognito}) que eliminan la barra de direcci\'on, las pesta\~nas, los di\'alogos de error y las notificaciones de actualizaci\'on. El resultado es que la pantalla muestra \'unicamente el contenido del dashboard, como una aplicaci\'on de pantalla completa que no se puede cerrar accidentalmente.
    \item \textbf{systemd}: Los servicios \texttt{xorg-kiosk.service} y \texttt{kiosk.service} se configuran con \texttt{Restart=always} y \texttt{RestartSec=10}, lo que significa que si Chromium crashea o la pantalla se desconecta, el sistema autom\'aticamente reinicia el servicio en 10 segundos sin intervenci\'on manual.
...
El sistema se encuentra \textbf{operativo y corriendo en producci\'on} con datos reales del inversor Riello H.P.6065REL-D. El daemon siser-reader (Python) se comunica v\'ia protocolo SISER sobre RS232, ejecuta el handshake autom\'aticamente, y almacena los datos en TimescaleDB. Grafana consulta la base de datos mediante SQL directo y muestra los 4 dashboards en tiempo real.

Todos los registros del mapa est\'an implementados con direcciones conocidas y se leen c\'iclicamente en producci\'on. Los registros base (temperatura, potencia, estado, energ\'ia total) provienen de Riello-RSTool, y los registros extendidos (voltajes, corrientes, frecuencia, acumulados) provienen de estimaciones del autor basadas en el an\'alisis del simulador. Estos \'ultimos se confirmar\'an cuando se conecte el inversor H.P.6065REL-D real v\'ia RS232.
...
\midrule
\multicolumn{6}{l}{\textit{Registros base (Riello-RSTool RS 3.0)}} \\
0x003C--0x003D & unlock password & 2 & --- & Cada reconexi\'on & \textbf{Activo} \\
0x101C & temperature & 1 & $\times 1.0$ & 5s & \textbf{Activo} \\
0x1037 & power\_ac & 2 & $\times 0.01$ & 5s & \textbf{Activo} \\
0x1005 & status & 1 & $\times 1.0$ & 5s & \textbf{Activo} \\
0x1021 & energy\_total & 2 & $\times 0.01$ & 60s & \textbf{Activo} \\
0xC000 & daily\_graph & 48 & $\times 0.01$ & 1x/d\'ia & \textbf{Activo} \\
\midrule
\multicolumn{6}{l}{\textit{Registros extendidos (estimados, confirmaci\'on con inversor real)}} \\
0x1040 & vpv & 1 & $\times 0.1$ & 60s & Estimado \\
0x1041 & ipv & 1 & $\times 0.01$ & 60s & Estimado \\
0x1042 & vac & 1 & $\times 0.1$ & 60s & Estimado \\
0x1043 & iac & 1 & $\times 0.01$ & 60s & Estimado \\
0x1044 & fac & 1 & $\times 0.01$ & 60s & Estimado \\
0x1060 & energy\_daily & 1 & $\times 0.01$ & 60s & Estimado \\
0x1050 & hours\_total & 2 & $\times 0.01$ & 60s & Estimado \\
0x1052 & co2\_saved & 2 & $\times 0.01$ & 60s & Estimado \\
\bottomrule
\end{tabular}
\caption{Mapa de registros Modbus RTU completo}
\label{tab:registers-pending}
\end{table}

\begin{notebox}
Los registros extendidos (0x1040--0x1052) son estimaciones del autor basadas en el an\'alisis del protocolo y se leer\'an del inversor H.P.6065REL-D v\'ia RS232 para confirmar o ajustar las direcciones actualizando \'unicamente el array \texttt{register\_map[]} en \texttt{register\_map.c}.
\end{notebox}
...
\begin{enumerate}
    \item \textbf{Conexi\'on serial}: El daemon abre \texttt{/dev/inverter-serial} (symlink udev al adaptador USB-RS232) a 9600 baud, 8N1, slave 1.
    \item \textbf{Unlock}: Env\'ia FC10 (Write Multiple Registers) a 0x003C--0x003D con valores [0x0000, 0x0000]. Sin este paso, el inversor no responde a FC03.
    \item \textbf{Lectura realtime (cada 5s)}: Lee los registros de la tabla \texttt{realtime} (temperatura 0x101C, potencia AC 0x1037, estado 0x1005) v\'ia FC03. Los valores crudos se multiplican por el factor de escala y se insertan en TimescaleDB.
    \item \textbf{Lectura fast\_samples (cada 60s)}: Lee los registros de las tablas \texttt{fast\_samples} y \texttt{cumulatives} (energ\'ia total 0x1021, voltajes 0x1040--0x1044, acumulados 0x1050--0x1060).
    \item \textbf{Lectura daily\_graph (1x/d\'ia)}: Lee 48 registros desde 0xC000 (gr\'afico de producci\'on por media hora).
    \item \textbf{Buffer circular}: Si TimescaleDB no est\'a disponible, las lecturas se acumulan en un buffer circular de 1000 muestras en RAM. Al restaurar la conexi\'on, el buffer se vac\'ia autom\'aticamente.
    \item \textbf{Reconexi\'on autom\'atica}: Si el adaptador USB se desconecta (error ENXIO o EIO), el daemon reintenta con backoff exponencial: 5s $\rightarrow$ 10s $\rightarrow$ 30s $\rightarrow$ 60s $\rightarrow$ 5 min m\'aximo.
    \item \textbf{Heartbeat}: Cada $\sim$90 segundos (si el inversor no responde), siser-reader inserta un registro con \texttt{is\_stale=true} para indicar que el inversor est\'a apagado (comportamiento nocturno normal).
\end{enumerate}

\begin{lstlisting}[caption={Salida t\'ipica del daemon en producci\'on}]
[2026-06-17 10:00:01] [INFO] siser-reader starting...
[2025-06-17 10:00:01] [INFO] Modbus context created: /dev/inverter-serial 9600 N81 slave=1
[2025-06-17 10:00:01] [INFO] Connected to serial port
[2025-06-17 10:00:01] [INFO] Inverter unlocked (Modbus protocol access granted)
[2025-06-17 10:00:01] [INFO] Connected and unlocked successfully
[2025-06-17 10:00:06] [INFO] realtime: temperature=42.0C power_ac=3200W status=2
[2025-06-17 10:00:11] [INFO] realtime: temperature=42.1C power_ac=3198W status=2
[2025-06-17 10:01:01] [INFO] fast: energy_total=12547.32kWh
\end{lstlisting}

\subsection{Confirmaci\'on con Inversor Real}

El sistema opera actualmente con el \textbf{simulador Modbus RTU} del inversor, que genera datos realistas (curva solar sinusoidal con ruido gaussiano). Todos los registros est\'an implementados y se leen c\'iclicamente. Los registros base provienen de Riello-RSTool y los extendidos son estimaciones del autor. Cuando se conecte el inversor H.P.6065REL-D v\'ia RS232, se ejecutar\'an las siguientes validaciones:

\begin{enumerate}
    \item \textbf{Confirmar registros base}: Verificar que las direcciones 0x101C (temperatura), 0x1037 (potencia AC), 0x1005 (estado) y 0x1021 (energ\'ia total) devuelven valores coherentes desde el inversor real.

    \item \textbf{Confirmar registros extendidos}: Verificar que las direcciones 0x1040--0x1052 (vpv, ipv, vac, iac, fac, energ\'ia diaria, horas, CO2) corresponden a las variables esperadas. Si difieren, se usar\'a \texttt{modbus\_scan.py} para escanear el rango 0x0000--0xFFFF y encontrar las direcciones correctas.

    \item \textbf{Captura de tr\'afico SunVision}: Con un Y-splitter RS232 y dos adaptadores USB-RS232, capturar qu\'e registros lee SunVision durante una sesi\'on completa. Esto revelar\'a el mapa completo de registros que Riello usa internamente.

    \item \textbf{Lecturas bajo condiciones variables}: Cambiar condiciones operativas (sombrear paneles, desconectar red AC, variar carga) y observar qu\'e registros cambian, para confirmar la correspondencia registro$\rightarrow$variable f\'isica.
\end{enumerate}

\begin{warnbox}
El simulador usa direcciones estimadas para los registros extendidos (0x1040 vpv, 0x1041 ipv, 0x1042 vac, 0x1043 iac, 0x1044 fac, 0x1050 horas, 0x1052 co2). Estas direcciones se usar\'an como punto de partida para la validaci\'on con el inversor real.
\end{warnbox}
...
\midrule
Comunicaci\'on Modbus RTU & mbpoll + pymodbus & El inversor responde a FC03/FC10, el unlock funciona, los valores le\'idos son coherentes & Post-deploy \\
...
Uptime & $>$99.5\% & Equivale a menos de 4.4 horas de ca\'ida al a\~no. Se logra con \texttt{restart: unless-stopped} en Docker y backoff exponencial en el daemon. \\
...
CPU (Modbus Reader) & $<$1\% & El daemon duerme 1 s entre ciclos, solo lee 3--5 registros por ciclo. El c\'odigo C compilado con libmodbus consume m\'inima CPU. \\
RAM (Modbus Reader) & $<$5 MB & Un proceso C sin malloc din\'amico m\'as all\'a del buffer circular de 1000 muestras (4 bytes cada una = 4 KB). \\
...
\midrule
inverter-simulator & \textbf{Operativo} & Genera datos Modbus RTU simulando la curva solar diaria con ruido gaussiano. Responde a FC03, FC10 y protocolo NetMan UDP. \\
...
Pantalla kiosco & \textbf{Operativo} & Chromium en modo kiosco mostrando el dashboard de tiempo real en la pantalla del laboratorio 24/7. \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
    ...
    \item \textbf{Unlock del protocolo Modbus}: Se descubri\'o y document\'o la secuencia de unlock del inversor H.P.6065REL-D (escritura en 0x003C--0x003D), requisito no documentado por Riello.
...
\begin{enumerate}
    \item Confirmar registros base (0x101C, 0x1037, 0x1005, 0x1021) y extendidos (0x1040--0x1052) con el inversor H.P.6065REL-D encendido v\'ia RS232
    \item Hacer servicios remotos persistentes via systemd (ttyd, cmd-server, ngrok)
...
El daemon siser-reader est\'a leyendo datos reales del inversor desde junio 2026, con m\'as de 22.000 registros en TimescaleDB. Los datos muestran que solo MPPT2 tiene paneles conectados (VPV $\sim$230V, IPV $\sim$0.5A, PPV $\sim$115W), mientras que MPPT1 y MPPT3 est\'an sin conectar.
